...
AddEventHandler('onResourceStart', function(resource)
   if resource == GetCurrentResourceName() then
    sendNotification("Fishing Script Loaded ".. Config.Core)
   end
end)
...
function sendNotification(message)
    ...
    elseif Config.Core == "QBCore" then
        exports['qb-core']:GetCoreObject().Functions.Notify(message, 'info')
...
function StartFishing()
 if isPickingUp then
        sendNotification(Config.Locale.alreadyFishing)
        return
    end
    

    if not IsNearWater() then
        sendNotification(Config.Locale.notNearWater)
        return
    end
    

    lib.callback('fishing:hasOldKey', false, function(hasOldKey)
        if hasOldKey then
            isFishing = true            
            ...
            sendNotification(Config.Locale.noOldKey)
        end
    end)
end
...
RegisterNUICallback('minigameResult', function(data, cb)
    ...
    if success then
        ...
        sendNotification(Config.Locale.fishCaught)
        ...
        if reason == "rope_split" then
            ...
            sendNotification(Config.Locale.ropeSplit)
        elseif reason == "fish_escaped" then
            PlayFishEscapedAnimation()
            sendNotification(Config.Locale.fishEscaped)
        else
            PlayFailAnimation()
            sendNotification(Config.Locale.fishingFailed)
        end
        isFishing = false
    end
...
function PickUpFish()
    ...
    sendNotification(Config.Locale.fishPickedUp)
...
function UpdateMarketInventory()
    lib.callback('fishing:getMarketInventory', false, function(items)
        SendNUIMessage({
